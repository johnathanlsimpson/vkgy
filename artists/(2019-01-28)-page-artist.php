<?php
	if(is_array($artist) && !empty($artist)) {
		...
		script([
			...
			"/scripts/script-likeComment.js",
			"/artists/script-page-artist.js"
		]);
		
		echo $_SESSION['username'] === 'inartistic' ? print_r($artist, true) : null;
		...
		?>
			...
			<div class="col c4-ABBB">
				<div class="artist__nav">
					<ul class="ul--compact">
						<?php
							echo is_array($artist['musicians'][1]) ? '<li><a href="#lineup">Lineup <br /> <span class="any--jp any--weaken">メンバー</span></a></li>' : null;
							echo is_array($artist['musicians'][2]) ? '<li><a href="#former">Former <br /> <span class="any--jp any--weaken">元メンバー</span></a></li>' : null;
							echo is_array($artist['musicians'][3]) ? '<li><a href="#staff">Staff <br /> <span class="any--jp any--weaken">スタッフ</span></a></li>' : null;
							echo is_array($artist['history']) ?      '<li><a href="#history">History <br /> <span class="any--jp any--weaken">活動</span></a></li>' : null;
							echo is_array($artist['schedule']) ?     '<li><a href="#schedule">Lives <br /> <span class="any--jp any--weaken">ライブ</span></a></li>' : null;
							echo                                     '<li><a href="#comments">Comment <br /> <span class="any--jp any--weaken">コメント</span></a></li>';
						?>
					</ul>
				</div>
										...
										?>
											...
											<?php
												if($musicians_type > 1) {
													?>
														<h2>
															<?php echo $musicians_type === 1 ? 'Lineup' : ($musicians_type === 2 ? 'Former members' : 'Staff'); ?>
															<div class="any--jp any--weaken">
																<?php echo $musicians_type === 1 ? 'メンバー' : ($musicians_type === 2 ? '元メンバー' : 'スタッフ'); ?>
															</div>
														</h2>
													<?php
												}
													...
													foreach($musicians as $musician) {
														?>
															<ul>
																...
																<ul class="ul--inline lineup__container">
																	<?php
																		foreach($musician["history"] as $history_chunk) {
																			?>
																				<ul class="ul--inline lineup__period any--weaken-color symbol__next">
																					<?php
						...
						<div class="artist__right">
							...
							<?php
								...
								if(!empty($artist["official_links"])) {
									...
									$many_links = count($artist["official_links"]) > 3 ? true : false;
										...
										?>
											<div>
												...
												<?php
													if($many_links) {
														?>
															<input class="any--hidden any__partial-input" id="show-all-links" type="radio" />
															<label class="any__partial-label input__button" for="show-all-links">
																Show all
															</label>
														<?php
													}
												...
												<div class="any--weaken text text--outlined <?php echo $many_links ? "any__partial" : null; ?>">
													<ul>
														<?php
								...
								<div>
									...
									<div class="text text--outlined">
										<form action="/php/function-add_comment.php" enctype="multipart/form-data" method="post" name="form__comment">
											<input name="item_type" type="hidden" value="artist" />
											<input name="item_id" type="hidden" value="<?php echo $artist["id"]; ?>" />
											...
											<span class="any--hidden"><?php echo comment_template([2 => "/user/".$_SESSION["username"]."/", 3 => $_SESSION["username"]]); ?></span>
										</form>
										...
										<form action="" class="any--flex any--flex-space-between any--signed-out-only comment__sign-in sign-in__container" enctype="multipart/form-data" method="post" name="sign-in__form">
											...
											<div class="input__row">
												<span class="input__group">
													...
													<span data-role="status"></span>
													<button class="any--hidden" name="submit" type="submit">
														Sign in
													</button>
												</span>
											</div>
										...
										<ul class="comment__container">
											<?php
												if(is_array($artist["comments"])) {
													foreach($artist["comments"] as $comment_thread) {
