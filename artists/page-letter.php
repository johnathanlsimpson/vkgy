<?php
	...
	$list_template = str_replace(["\t", "\r", "\n"], "", '
		<li class="artist-list__item" data-recent="{8}" data-is_exclusive="{9}">
			<div class="h5 artist-list__history">{1}</div>
			<a class="artist artist-list__link artist artist--no-symbol any--flex" data-name="{2}" data-quickname="{3}" href="{4}">
				<span class="artist-list__name">{5}</span> <span class="any__note artist-list__hint">{6}</span>
			</a>
		</li>
	');
					...
					for($n=0; $n<$num_artists; $n++) {
						echo str_replace(
							...
							[
								...
								($artist_list[$n]["romaji"] ? lang($artist_list[$n]['romaji'], $artist_list[$n]['name'], ['container' => 'div']) : $artist_list[$n]["name"]),
