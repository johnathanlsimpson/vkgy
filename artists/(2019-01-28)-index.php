<?php	
	...
	elseif($show_artist_page) {
		...
		for($a=1; $a<=$num_musician_types; $a++) {
			...
			for($b=0; $b<$num_musicians; $b++) { echo $_SESSION['username'] === 'inartistic' ? '-' : null;
				...
				for($c=0; $c<$num_history_periods; $c++) {
					...
					for($d=0; $d<$num_bands; $d++) {
						...
						if($artist['musicians'][$a][$b]['history'][$c][$d]['is_session']) {
							...
							$artist['musicians'][$a][$b]['history']['sessions'][] = $artist['musicians'][$a][$b]['history'][$c][$d];
		...
		//echo $_SESSION['username'] === 'inartistic' ? '<pre>'.print_r($artist['musicians'], true).'</pre>' : null;
		...
		$artist["comments"] = $access_comment->access_comment(["id" => $artist["id"], "type" => "artist", "get" => "all"]);
