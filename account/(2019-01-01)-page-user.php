<?php
	...
	include_once("../avatar/avatar-definitions.php");
		...
		for($i=0; $i<$num_collected; $i++) {
			$collection[$i]["artist"] = $access_artist->access_artist(["id" => $collection[$i]["artist_id"], "get" => "name"]);
			$total_cost = $total_cost + (is_numeric($collection[$i]["price"]) ? $collection[$i]["price"] : 0);
		}
			...
			<div class="col c3-ABB">
				...
				<div>
					...
					<div class="collection__wrapper text" id="collection-wrapper">
						...
						<table class="collection__container" id="collection-container">
							<?php
								if(is_array($collection) && !empty($collection)) {
									...
									for($i=0; $i<$num_collected; $i++) {
										...
										?>
											<tr class="<?php echo $collection[$i]["is_for_sale"] ? "collection--for-sale" : "collection--not-for-sale"; ?>">
												<td>
