<?php
	...
	class vkgy_image {
				...
				if(strlen($resized_image) && file_exists($resized_image)) {
					...
					if($resized_image === $watermarked_path) {
						...
						if(strtolower(pathinfo($watermarked_path, PATHINFO_EXTENSION)) === 'jpg') {
							...
							// Should be jpg after resizing, I think?
							/*if($source_extension === 'png') {
								if($watermarked_image = imagecreatefrompng($watermarked_path)) {
								}
							}
							elseif($source_extension === 'gif') {
								if($watermarked_image = imagecreatefromgif($watermarked_path)) {
								}
							}
							elseif($source_extension === 'jpg' || $source_extension === 'jpeg') {
								if($watermarked_image = imagecreatefromjpeg($watermarked_path)) {
								}
							}*/
							...
							if(is_resource($watermarked_image)) {
								...
								if(imagejpeg($watermarked_image, $watermarked_path, 100)) {
									...
									// Image could not be watermarked or saved so... try to show large, then 404?
									$output_image_path = $this->image_paths['full'].$source_id.'.'.'jpg';
								}
								
								/*// This saves the watermarked image, but I think we want all watermarked images to be saved as jpg?
								if($ext === "png") {
		...
		function get_resized_image($source_id, $source_extension, $resize_method) {
			//if(strlen($source_image_path)) {
			if(is_numeric($source_id)) {
				//if(strlen($source_extension)) {
				...
				if(file_exists($full_path)) {
					if($source_extension === 'gif') {
						// Return source image path, since gifs aren't resized
						$output_image_path = $full_path;
					}
					elseif(file_exists($resized_path)) {
						// Return already-made resized file
		...
		function get_full_size_image($source_id, $source_extension) {
			...
			if(is_array($rslt_image) && !empty($rslt_image)) {
				...
				if($image_is_exclusive) {
					...
					if(
						(!$image_is_exclusive) ||
						($image_is_exclusive && $user_is_uploader && $_SESSION['username'] != 'inartistic') ||
						($image_is_exclusive && $user_is_vip && $_SESSION['username'] != 'inartistic') ||
	...
	$test = new vkgy_image($pdo);
	$test->get_image([ 'artist' => $_GET['artist'], 'method' => $_GET['method'], 'image_path_only' => false ]);
	//$test->get_artist_image_id($_GET['artist'])
				//session_cache_limiter('public');
				...
				//header('Expires: '.gmdate('D, d M Y H:i:s', time() + 120).' GMT');
				//header('Cache-Control: max-age=120');
//readfile('../images/image_files_thumbnail/5105.jpg');//
//unlink('../images/image_files_watermarked/5105.jpg');
