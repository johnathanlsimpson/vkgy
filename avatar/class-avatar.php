									...
									if(is_array($shape)) {
										if(!$part_attributes["extends_part"] || isset($this->filtered_avatar_options[$layer_name][$part_attributes["extends_part"]]["shapes"][$shape_name])) {
											...
											if($part_attributes['extends_part'] && !isset($this->filtered_avatar_options[$layer_name][$part_name]["colors"])) {
												$color = $shape["fill"] ?: $this->filtered_avatar_options[$layer_name][$part_attributes['extends_part']]["colors"];
											}
											else {
											...
											$fill  = $fill ?: (strpos($shape["custom"], "fill=") === false ? 'fill="'.$color.'"' : null);
											
											if($shape["custom"]) {
												echo '<'.$shape["custom"].' class="'.$class.' '.$shape["class"].'" clip-path="'.$clip_path.'" '.$fill.' />';
											}
											else {
												?>
													<path class="<?php echo $class.' '.$shape["class"]; ?>" clip-path="<?php echo $clip_path; ?>" d="<?php echo $shape["path"]; ?>" <?php echo $fill; ?> transform="<?php echo $transform; ?>" />
												<?php
											}
										}
									}
