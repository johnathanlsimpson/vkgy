...
function fetchParams ()/*{{{ Get parameters from bitbucket payload now only (REPO) */
{
    ...
    foreach ( $PAYLOAD->push->changes as $change ) {
			_LOG("Is object?".(is_object($change->new) ? "yes" : "no"));
			_LOG("Type branch?".($change->new->type == "branch" ? "yes" : "no"));
			_LOG("Isset ".print_r($PROJECTS[$REPO], true)."*".print_r($change->new->name, true)."?".(isset($PROJECTS[$REPO][$change->new->name]) ? "yes" : "no"));
...
{
    global $REPO, $REPO_NAME, $CONFIG, $BRANCHES, $PAYLOAD;
    ...
    if ( $CONFIG['verbose'] ) {
        ...
        _LOG_VAR('BRANCHES',$BRANCHES);
				_LOG_VAR('PAYLOAD',print_r($PAYLOAD, true));
    }
}/*}}}*/
