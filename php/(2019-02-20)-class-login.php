<?php
	...
	class login {
			...
			private $pdo;
			private $domain;
			private $secret_key = "Mar:vior.2013";
			public  $status;
			...
			public function sign_in($input) {
				...
				if(preg_match("/".$username_pattern."/", $input["username"])) {
					$sql_user = "SELECT id, username, rank, icon, password_old, password, site_theme FROM users WHERE username=? LIMIT 1";
			...
			public function sign_out() {
				foreach(["userID", "username", "admin", "icon", "hash", "loggedIn", 'site-theme', "remember_me"] as $key) {
					unset($_SESSION[$key]);
