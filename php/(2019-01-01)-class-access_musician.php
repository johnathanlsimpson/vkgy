<?php
	...
	class access_musician {
			...
			if(!empty($band_history)) {
				...
				if(is_array($band_history_periods)) {
					foreach($band_history_periods as $period_key => $band_history_period) {
						...
						$bands = explode(", ", $band_history_period);
						if(is_array($bands)) {
		...
		function access_musician($args = []) {
			...
			if($args["get"] === "list") {
				array_push($sql_select, "musicians.id", "musicians.usual_position", "musicians.history", "musicians.name", "musicians.romaji", "COALESCE(musicians.romaji, musicians.name) AS quick_name", "musicians.friendly", "musicians.edit_history");
			}
			if($args["get"] === "all") {
