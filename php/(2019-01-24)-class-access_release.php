<?php
	...
	class access_release {
		...
		function access_release($args = []) {
			...
			// [PRE-SELECT] Edit history
			if($args["edit_history"]) {
				/*$sql_pre = 'SELECT edits_releases.release_id, users.username FROM edits_releases LEFT JOIN users ON users.id=edits_releases.user_id WHERE edits_releases.date_occurred>? AND edits_releases.date_occurred<? ORDER BY edits_releases.date_occurred DESC '.($args["limit"] ? "LIMIT ".$args["limit"] : null);
				$stmt_pre = $this->pdo->prepare($sql_pre);
				$stmt_pre->execute([
					date("Y-m-d", strtotime("-1 month", strtotime($args["edit_history"]))),
					date("Y-m-d", strtotime("+1 month", strtotime($args["edit_history"])))
				]);
				$rslt_pre = $stmt_pre->fetchAll();
				...
				if(is_array($rslt_pre) && !empty($rslt_pre)) {
					...
					for($i = 0; $i < count($rslt_pre); $i++) {
						$args["release_id"][] = $rslt_pre[$i]['release_id'];
						$usernames[$rslt_pre[$i]['release_id']] = $rslt_pre[$i]['username'];
					}
					
					unset($rslt_pre, $args["edit_history"]);
				}
				else {
					unset($args["get"]);
				}
				
				echo $_SESSION['username'] === 'inartistic' ? '<pre>'.print_r($args['release_id'], true).'</pre>' : null;*/
			}
			...
			if(1) {
				...
				elseif($args["get"] === "list") {
					...
					if($args['edit_history']) {
						...
						$sql_select[] = 'edits_releases.date_occurred';
					}
			...
			/*if(!empty($args["edit_history"]) && preg_match("/"."\d{4}-\d{2}-\d{2}"."/", $args["edit_history"])) {
				if($args["edit_history"] < date("Y-m-d")) {
					$sql_where[] = "SUBSTRING(releases.edit_history, 1, 10) < ?";
					$sql_values[] = $args["edit_history"];
				}
				
				$sql_order[] = "releases.edit_history DESC";
				$sql_force = "FORCE INDEX (edit_history)";
			}*/
				...
				if(is_array($releases) && !empty($releases)) {
					for($i = 0; $i < $num_rslt_releases; $i++) {
						...
						if($args["get"] === "all") {
							...
							$releases[$release_id]["comments"] = $this->access_comment->access_comment(["id" => $releases[$release_id]["id"], "type" => "release", "get" => "all"]);
						}
					}
