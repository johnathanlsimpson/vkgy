							...
							if($stmt_update->execute([$extension, $id])) {
								if(rename($new_name, "../images/tmp/".$file_name)) {
									if($needs_compression && $extension !== "gif") {
										try {
											$source = \Tinify\fromFile("../images/tmp/".$file_name);
											$source->toFile(($queued ? "../images/image_files_queued/" : "../images/image_files/").$file_name);
									...
									if($queued) {
										$image_thumb = new \Eventviva\ImageResize("../images/image_files_queued/".$file_name);
									...
									$output["image_id"] = $id;
									$output["image_style"] = "background-image: url(/images/".$file_name.");";
