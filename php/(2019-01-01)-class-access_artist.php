	...
	class access_artist {
		...
		function access_artist($args = []) {
			...
			switch($args["get"]) {
				...
				case "list"        : array_push($sql_select, "artists.id", "artists.name", "artists.romaji", "COALESCE(artists.romaji, artists.name) AS quick_name", "artists.friendly", "artists.label_history", "SUBSTRING_INDEX(artists.edit_history,'\n',1) AS edit_history"); break;
				case "artist_list" : array_push($sql_select, "artists.id", "artists.name", "artists.romaji", "COALESCE(artists.romaji, artists.name) AS quick_name", "artists.friendly", "artists.is_exclusive", "SUBSTRING_INDEX(artists.edit_history,'\n',1) AS edit_history"); break;
			}
