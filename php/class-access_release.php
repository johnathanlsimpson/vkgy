...
include_once('../php/class-song.php');
			...
			if(is_array($input_tracklist)) {
				foreach($input_tracklist as $track) {
					...
					foreach(["name", "romaji"] as $key) {
						...
						// Replace escaped parentheses (handling various combinations of sanitize)
	...
	// ======================================================
	// Update romaji of any releases with same name as song
	// ======================================================
	public function update_releases_romaji_to_match_song( $original_name, $name, $romaji, $artist_id ) {
		
		if( is_numeric($artist_id) && strlen($name) ) {
			
			$romaji = strlen($romaji) ? $romaji : null;
			
			$sql_update = 'UPDATE releases SET name=?, romaji=? WHERE name=? AND artist_id=?';
	...
	// ======================================================
	// Clean common mistakes in tracks
	// ======================================================
	public function clean_track_mistakes( $name, $romaji ) {
		
		// Attempt to find if Japanese/romaji were switched
		$jp_pattern = '\p{Han}|\p{Katakana}|\p{Hiragana}|\p{Hangul}';
		if( strlen( $name ) && strlen( $romaji ) && !preg_match( '/'.$jp_pattern.'/u', $name ) && preg_match( '/'.$jp_pattern.'/u', $romaji ) ) {
			$temp_romaji = $romaji;
			$temp_name   = $name;
			$name        = $temp_romaji;
			$romaji      = $temp_name;
		...
		// Unset romaji if it's exactly the same as name
		if( $romaji === $name ) {
			$romaji = null;
		}
		
		// Often not correctly marked as note
		$should_be_notes = [
			'告知用Music Video',
			'Kokuchiyou Music Video',
			'Making of PV',
			'Making of MUSIC CLIP',
			'MUSIC CLIP complete version',
			'ライヴ映像',
			'LIVE Eizou',
			'ダイジェスト映像',
			'DIGEST Eizou',
		...
		// For both name and romaji, loop through and fix other mistakes
		// The anonymous array/pass by array thing is probably dumb but it works so idk
		foreach([ &$name, &$romaji ] as &$input) {
			
			// Undo character entities
			//$input = html_entity_decode( $input, ENT_QUOTES, 'UTF-8' );
			
			// Loop through each simple note replacement and wrap it in parentheses
			foreach( $should_be_notes as $should_be_note ) {
				
				//$pattern_to_match = '(?:^| )([\[\-]?'.$should_be_note.'[\]\-]?)(?: |$)';
				$pattern_to_match = '([\[\-\ ]?'.$should_be_note.'[\]\-]?)$';
				
				// To avoid fucking things up with multi-replacements, stop after the first match/replace
				if( preg_match('/'.$pattern_to_match.'/iu', $input) ) {
					
					$input = preg_replace( '/'.$pattern_to_match.'/iu', ' ('.$should_be_note.')', $input );
					break;
	...
	// =======================================================
	// Find notes in track, remove, and put into own strings
	// =======================================================
	public function split_notes_from_track( $track=[] ) {
		
		// Make sure we have a track
		if( is_array($track) && !empty($track) ) {
			
			// And let's not do anything if the notes columns are already populated
			if( !strlen($track['notes_name']) && !strlen($track['notes_romaji']) ) {
				
				// Find notes in the track
				$track_notes = $this->get_notes_from_track($track);
				
				// Create some blank placeholders
				$notes_name = [];
				$notes_romaji = [];
				
				// Loop through the found notes and decide what to do with them
				if( is_array($track_notes['notes']) && !empty($track_notes['notes']) ) {
					foreach( $track_notes['notes'] as $track_note_key => $track_note ) {
						
						// Save the JP notes
						$notes_name[] = $track_note['name'];
						
						// If the track has romaji but there's no romaji note, save the JP note as the romaji note
						if( strlen($track['romaji']) && !strlen($track_note['romaji']) ) {
							$notes_romaji[] = $track_note['name'];
						}
						
						// Otherwise, save whatever's listed as the romaji note
						else {
							$notes_romaji[] = $track_note['romaji'];
						}
						
						// If note is in middle of string, we don't want to end up with a mangled title (e.g. Orion (→) Pains would be Orion Pains)
						// So let's check if this current note ends in the middle of the track
						if( $track_note['name_offset'] + $track_note['name_length'] < strlen($track['name']) ) {
							
							// Now, if the note *is* in the middle of the track, but the track is actually a string of several notes, we're ok
							// and we can find this because '(note) (note)' is ok but '(note) abc (note)' is not
				...
				// Now, if we still have an array of notes, let's remove those notes from the track names
				if( is_array($notes_name) && !empty($notes_name) ) {
					foreach( $notes_name as $note_key => $note_name ) {
						
						$track['name'] = str_replace( '('.$note_name.')', '', $track['name'] );
						$track['romaji'] = str_replace( '('.$notes_romaji[ $note_key ].')', '', $track['romaji'] );
						
					}
				}
				
				// Now implode the notes back into a string so we can store in DB, separately from name
				$track['notes_name'] = implode( ') (', array_filter( $notes_name ) );
				$track['notes_name'] = $track['notes_name'] ? '('.$track['notes_name'].')' : null;
				$track['notes_romaji'] = implode( ') (', array_filter( $notes_romaji ) );
				$track['notes_romaji'] = $track['notes_romaji'] ? '('.$track['notes_romaji'].')' : null;
				
			}
			
			// Make sure each value is nulled out if necessary
			foreach( $track as $key => $value ) {
				$track[ $key ] = strlen($value) ? $value : null;
			}
	...
	// Update tracklist
	// =======================================================
	public function update_tracklist( $release_id, $tracklist=[], $date_occurred=null ) {
		
		// The release date will be used to make a better song guess and potentially update song
		...
		if( is_numeric($release_id) ) {
			...
			if( is_array($tracklist) && !empty($tracklist) ) {
				...
				// Set dummy fields for notes_name and notes_romaji since they're actually populated from name/romaji
				//$tracklist[0]['notes_name'] = null;
				//$tracklist[0]['notes_romaji'] = null;
				...
				foreach( $tracklist as $track_key => $track ) {
					...
					// Name and romaji might have been sanitized from function-update_release, so let's undo before we clean them up
					$track['name']   = html_entity_decode( $track['name'], ENT_QUOTES, 'UTF-8' );
					$track['romaji'] = html_entity_decode( $track['romaji'], ENT_QUOTES, 'UTF-8' );
					
					// Undo common mistakes such as flipping name/romaji or not commenting out PV
					list( $track['name'], $track['romaji'] ) = $this->clean_track_mistakes( $track['name'], $track['romaji'] );
					
					// Handle mistranslations of symbols
					$track['romaji'] = $this->sanitizer->match_romaji_to_japanese( $track['name'], $track['romaji'] );
					...
					if( !is_numeric( $track['song_id'] ) && !$track['is_custom'] ) {
						...
						$song = $this->access_song->guess_song([
							'name' => $track['name'],
							'romaji' => $track['romaji'],
					...
					if( is_numeric( $track['song_id'] ) && !$track['is_custom'] ) {
						...
						// In the future, if we have an exact song match we should clear the track title and romaji--but let's keep them until kinks are worked out
						//$track['name'] = null;
						//$track['romaji'] = null;
						
					}
					
					// If notes not explicitly set, try to find them
					if( !strlen($track['notes_name']) && !strlen($track['notes_romaji']) ) {
						
						// Find notes in track, remove them from track name, move them to notes_name
						$track = array_merge( $track, $this->split_notes_from_track($track) );
						
					}
					
					// Sanitize notes and romaji since they were raw before
					$track['name'] = sanitize($track['name']);
					$track['romaji'] = sanitize($track['romaji']);
					$track['notes_name'] = sanitize($track['notes_name']);
					$track['notes_romaji'] = sanitize($track['notes_romaji']);
					$track['is_custom'] = $track['is_custom'] ? 1 : 0;
					$track['song_id'] = is_numeric($track['song_id']) ? $track['song_id'] : null;
	...
	function access_release($args = []) {
		...
		if( is_numeric($args['song_id']) ) {
			$sql_from = 'releases_tracklists';
			$sql_join[] = 'LEFT JOIN releases ON releases.id=releases_tracklists.release_id';
		}
		...
		// Get by song
		if( is_numeric($args['song_id']) ) {
			$sql_where[] = 'releases_tracklists.song_id=?';
		...
		if(is_array($sql_select) && !empty($args["get"])) {
			...
			if($args["get"] === "basics" || $args["get"] === "all") {
				...
				SELECT 
					releases_tracklists.release_id, 
					releases_tracklists.song_id, 

					IF( 
						songs.id IS NOT NULL AND releases_tracklists.is_custom=0,
						IF( releases_tracklists.notes_name IS NOT NULL, CONCAT( songs.name, ' ', releases_tracklists.notes_name ), songs.name ),
						CONCAT( COALESCE( releases_tracklists.name, '' ), IF( releases_tracklists.name IS NOT NULL AND releases_tracklists.notes_name IS NOT NULL, ' ', '' ), COALESCE( releases_tracklists.notes_name, '' ) )
					) AS name,

					IF(
						songs.id IS NOT NULL AND releases_tracklists.is_custom=0,
						IF(
							songs.romaji IS NOT NULL OR releases_tracklists.notes_romaji IS NOT NULL,
							CONCAT(
								COALESCE( songs.romaji, songs.name ),
								IF( releases_tracklists.notes_name IS NOT NULL, ' ', '' ),
								COALESCE( releases_tracklists.notes_romaji, releases_tracklists.notes_name, '' )
							),
							''
						),
						CONCAT( COALESCE( releases_tracklists.romaji, '' ), IF( releases_tracklists.romaji IS NOT NULL AND releases_tracklists.notes_romaji IS NOT NULL, ' ', '' ), COALESCE( releases_tracklists.notes_romaji, '' ) )
					) AS romaji,

					releases_tracklists.disc_num, 
					releases_tracklists.disc_name, 
					releases_tracklists.disc_romaji, 
					releases_tracklists.section_num, 
					releases_tracklists.section_name, 
					releases_tracklists.section_romaji, 
					releases_tracklists.track_num, 
					releases_tracklists.artist_id, 
					releases_tracklists.artist_display_name, 
					releases_tracklists.artist_display_romaji,
					releases_tracklists.notes_name,
					releases_tracklists.notes_romaji,
					releases_tracklists.is_custom
				FROM 
					releases_tracklists 
				LEFT JOIN
					songs ON songs.id=releases_tracklists.song_id
				WHERE 
					releases_tracklists.release_id IN (".str_repeat("?,", count((is_array($args["release_id"]) ? $args["release_id"] : [$args["release_id"]])) - 1)."?".") 
				ORDER BY 
					releases_tracklists.release_id ASC, 
					releases_tracklists.id ASC, 
					releases_tracklists.disc_num ASC, 
					releases_tracklists.section_num ASC, 
					releases_tracklists.track_num ASC";
