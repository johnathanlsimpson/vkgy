	...
	class access_release {
		...
		function access_release($args = []) {
			...
			if($args["edit_history"]) {
				...
				$sql_pre = "SELECT id FROM releases WHERE edit_history>? AND edit_history<? ORDER BY edit_history DESC ".($args["limit"] ? "LIMIT ".$args["limit"] : null);
				$stmt_pre = $this->pdo->prepare($sql_pre);
				$stmt_pre->execute([
			...
			if(is_numeric($args["user_id"])) {
				...
				//$sql_select[] = "releases.press_limitation_num";
				//$sql_select[] = "releases.venue_limitation";
				//$sql_select[] = "releases_collections.date_occurred AS date_collected";
			}
			...
			if($args["get"] === "list") {
				//$sql_from[] = "LEFT JOIN images ON images.release_id=CONCAT('(', releases.id, ')') AND images.is_release='1' AND images.is_default='1'";
			}
