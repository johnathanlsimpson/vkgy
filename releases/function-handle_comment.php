<?php
	...
	if($method === "delete") {
		if($_SESSION["loggedIn"] && is_numeric($comment_id)) {
	...
	elseif($method === "like") {
		$sql_check_comment_like = "SELECT 1 FROM releases_likes WHERE ".($_SESSION["loggedIn"] ? "user_id" : "ip_address")."=? AND comment_id=? LIMIT 1";
		$stmt = $pdo->prepare($sql_check_comment_like);
		$stmt->execute([$_SESSION["loggedIn"] ? $user_id : $ip_address, $comment_id]);
		...
		if($like_action === "add") {
			$sql_like = "INSERT INTO releases_likes (".($_SESSION["loggedIn"] ? "user_id" : "ip_address").", comment_id) VALUES (?, ?)";
		}
		else {
			$sql_like = "DELETE FROM releases_likes WHERE ".($_SESSION["loggedIn"] ? "user_id" : "ip_address")."=? AND comment_id=? LIMIT 1";
		}
		
		$stmt = $pdo->prepare($sql_like);
		
		if($stmt->execute([$_SESSION["loggedIn"] ? $user_id : $ip_address, $comment_id])) {
			$output["status"] = "success";
			$output["action"] = $like_action;
		}
	}
	else {
		if($_SESSION["loggedIn"] && is_numeric($release_id) && !empty($comment_content)) {
