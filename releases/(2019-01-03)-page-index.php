...
<?php
	...
	$edited_releases = $access_release->access_release([ "edit_history" => date("Y-m-d"), "get" => "list", "limit" => "20" ]);
								...
								foreach($day as $release) {
									...
									?>
										<div 
											...
											<a class="calendar__cover" href="<?php echo str_replace('.small', '', ($cover_image ?: $artist_image)); ?>" target="_blank" title="<?php echo ($cover_image ? '&ldquo;'.$release['quick_name'].'&rdquo; cover' : $release['artist']['quick_name'].' image'); ?>">
								...
								<div class="text">
									<ul>
										<?php
											...
											for($i = 0; $i < count($edited_releases); $i++) {
												?>
													<li>
														<div class="h5">
															<?php
																$edit_history = strtok($edited_releases[$edited_release_ids[$i]]["edit_history"], "\n");
																$edit_user_id = substr($edit_history, 21, -1);
																$edit_history = substr($edit_history, 0, 10);

																if(!is_array($users[$edit_user_id])) {
																	$users[$edit_user_id] = $access_user->access_user(["id" => $edit_user_id, "get" => "name"]);
																}

																echo $edit_history . " by " . '<a class="user a--inherit" href="/users/'.$users[$edit_user_id]["username"].'/">'.$users[$edit_user_id]["username"].'</a>';
															?>
														</div>
														<div class="">
															<a class="artist any--weaken-size" href="/releases/<?php echo $edited_releases[$edited_release_ids[$i]]["artist"]["friendly"]; ?>/">
																<?php echo $edited_releases[$edited_release_ids[$i]]["artist"]["quick_name"]; ?>
															</a>
															<br />
															<a class="symbol__release" href="/releases/<?php echo $edited_releases[$edited_release_ids[$i]]["artist"]["friendly"]."/".$edited_releases[$edited_release_ids[$i]]["id"]."/".$edited_releases[$edited_release_ids[$i]]["friendly"]; ?>/">
																<?php echo $edited_releases[$edited_release_ids[$i]]["quick_name"]; ?>
															</a>
														</div>
													</li>
												<?php
