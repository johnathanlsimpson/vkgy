<?php
	...
	/* Get comments */
	$comments = $access_comment->access_comment(["get" => "list", "limit" => 20]);
	...
	/* Updates */
	$sql_select_recent = "COALESCE(romaji, name) AS quick_name, SUBSTRING_INDEX(edit_history, '\n', 1) AS edit_history_1, SUBSTRING(edit_history, 0, 100) AS edit_history_2, '' AS artist_friendly, '' as artist_quick_name";
	$sql_end_recent = "ORDER BY edit_history DESC LIMIT 7";
	
	$sql_recent = "
		SELECT *, SUBSTRING(edit_history, 1, 10) AS date_occurred
		FROM (
			(
				SELECT
					'artist' AS type,
					...
					SUBSTRING(artists.edit_history, 1, 19) AS edit_history,
					'' AS artist_url,
					'' AS artist_quick_name
				FROM artists
				ORDER BY artists.edit_history DESC LIMIT 7
			)
			UNION
			(
				SELECT
					...
					SUBSTRING(labels.edit_history, 1, 19) AS edit_history,
					'' AS artist_url,
					'' AS artist_quick_name
				FROM labels
				ORDER BY labels.edit_history DESC LIMIT 7
			)
			UNION
			(
				SELECT
					...
					SUBSTRING(releases.edit_history, 1, 19) AS edit_history,
				...
				ORDER BY releases.edit_history DESC LIMIT 7
			)
		) updates
		ORDER BY edit_history DESC
	";
...
<div class="col c4-AAAB any--margin">
	<div>
		<p class="any--weaken">
			<strong><a class="a--inherit" href="https://vk.gy/">vkgy</a></strong> is a visual kei library maintained by overseas fans. It specializes in small bands, and hard-to-find information.
		</p>
		<p class="any--jp any--weaken">
			<a class="a--inherit" href="https://vk.gy/">vkgy</a>（ブイケージ）はビジュアル系のライブラリです。関連するアーティストのメンバープロフィールや活動やリリース情報などがあります。
		</p>
	</div>
	
	<div class="cta__container">
		<a class="a--padded a--outlined cta__link" href="https://patreon.com/vkgy/" target="_blank">Support vkgy</a>
	</div>
</div>
					...
					<?php
						for($i=0; $i<$num_updates; $i++) {
							?>
								<li>
									<h5>
										<?php echo $updates[$i]["date_occurred"]; ?>
									</h5>
