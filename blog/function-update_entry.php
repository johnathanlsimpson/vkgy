	...
	if(strlen($title) && strlen($friendly) && strlen($content)) {
		if($is_allowed) {
			if($friendly_is_allowed) {
				...
				// If no images supplied, but main artist specified, grab main artist's default photo
				if(!is_numeric($_POST['image_id']) && is_numeric($artist_id)) {
					
					// Attempt to get image ID of entry's main artist
					$sql_image = 'SELECT artists.image_id FROM artists WHERE artists.id=? LIMIT 1';
					$stmt_image = $pdo->prepare($sql_image);
					$stmt_image->execute([ $artist_id ]);
					$rslt_image = $stmt_image->fetchColumn();
					
					// If found image for artist, add to blog query
					if(is_numeric($rslt_image)) {
						$keys_blog[] = 'image_id';
						$values_blog[] = $rslt_image;
					}
				...
				if($stmt_blog->execute($values_blog)) {
					...
					// If default image was automatically set from artist image, make sure connections are set up
					...
					if(!$is_edit || ($is_edit && !$extant_social_post) || (is_array($extant_social_post) && is_numeric($extant_social_post['id']) && !$extant_social_post['is_completed'] )) {
						...
						if(!$is_queued && strlen($title) && strlen($friendly)) {
							...
							// Convert contributor IDs to string to we can escape
							$contributor_ids = is_array($_POST['contributor_ids']) ? implode(',', $_POST['contributor_ids']) : $_POST['contributor_ids'];
							...
							$social_post = $access_social_media->build_post([
								...
								'artist_id'                 => $artist_id,
								'user_id'                   => $user_id,
								'contributor_ids'           => explode(',', sanitize($contributor_ids)),
