...
class grassthread_scraper {
	...
	/*	$scraper = curl_init();
		curl_setopt($scraper, CURLOPT_URL, $url);
		curl_setopt($scraper, CURLOPT_RETURNTRANSFER, true);
		curl_setopt($scraper, CURLOPT_PROXY, $proxy);
 curl_setopt($scraper, CURLOPT_SSL_VERIFYPEER, 0);
 curl_setopt($scraper, CURLOPT_SSL_VERIFYHOST, 0);
 $httpCode = curl_getinfo($scraper , CURLINFO_HTTP_CODE); // this results 0 every time
		$document = curl_exec($scraper);
		
		print_r(curl_version());
		print_r(file_get_contents(str_replace('https://vk.gy/', '../', $url)));
		//curl_close($scraper);
		
 //$response = curl_exec($ch);
 if ($document === false) $response = curl_error($scraper);
