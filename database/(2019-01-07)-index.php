...
$sql_recent = "
	...
	FROM (
		...
		(
			...
			ORDER BY edits_labels.date_occurred DESC LIMIT 20
		)
		UNION
		(
			SELECT 'musician' AS type, edits_musicians.user_id, CONCAT_WS('/', '', 'musicians', musicians.id, musicians.friendly, '') AS url, COALESCE(romaji, name) AS quick_name, edits_musicians.date_occurred AS date_edited, '' AS artist_quick_name, '' AS artist_url
			FROM edits_musicians
			LEFT JOIN musicians ON musicians.id=edits_musicians.musician_id
			GROUP BY edits_musicians.musician_id
			ORDER BY edits_musicians.date_occurred DESC LIMIT 20
		)
		UNION
		(
