							...
							foreach($comments as $comment_thread) {
								...
								foreach($comment_thread as $comment) {
									$rendered_comments[] = render_comment_component($comment_template, [
										...
										'content' => !$comment['is_deleted'] && ($comment['is_approved'] || (!$comment['is_approved'] && ($_SESSION['is_admin'] || $_COOKIE['anonymous_id'] === $comment['anonymous_id']))) ? $markdown_parser->parse_markdown($comment['content']) : '',
