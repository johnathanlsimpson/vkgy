							...
							foreach($comments as $comment_thread) {
								...
								foreach($comment_thread as $comment) {
									$rendered_comments[] = render_comment_component($comment_template, [
										...
										'is_user' => ($comment['anonymous_id'] && $_SESSION['anonymous_id'] === $comment['anonymous_id']) || ($_SESSION['loggedIn'] && $_SESSION['userID'] === $comment['user']['id']) ? '1' : '0',
										...
										'content' => !$comment['is_deleted'] && ($comment['is_approved'] || (!$comment['is_approved'] && ($_SESSION['admin'] || $_SESSION['anonymous_id'] === $comment['anonymous_id']))) ? $markdown_parser->parse_markdown($comment['content']) : '',
